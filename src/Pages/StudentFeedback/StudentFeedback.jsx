...
//     const res = await axios.get("https://assignment-11-server-site-ashen.vercel.app/feedbacks");
...
//     try {
//       const res = await axios.post("https://assignment-11-server-site-ashen.vercel.app/feedbacks", feedbackData);
...
//         await axios.delete(`https://assignment-11-server-site-ashen.vercel.app/feedbacks/${id}`);
//         fetchFeedbacks();
...
//       await axios.put(`https://assignment-11-server-site-ashen.vercel.app/feedbacks/${id}`, updatedFeedback);
...
const StudentFeedback = () => {
  ...
  const handleUpdate = async (id) => {
    try {
      const updatedFeedback = { message: editMessage };
      await axios.patch(`https://assignment-11-server-site-ashen.vercel.app/feedbacks/${id}`, {
        updatedData: updatedFeedback,
        userEmail: user.email
      });
      Swal.fire("Updated!", "Feedback Updated Successfully", "success");
      setEditingId(null);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {feedbacks.map((fb) => (
          <div key={fb._id} className="bg-gradient-to-br from-blue-50 via-white to-blue-100 rounded-2xl shadow-2xl p-4 border border-blue-200 relative group">
            ...
            {user.email === fb.email && editingId !== fb._id && (
              <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition">
                <button onClick={() => handleEdit(fb._id, fb.message)} className="text-blue-500 hover:text-blue-700">
                  <FaEdit size={20} />
                </button>
