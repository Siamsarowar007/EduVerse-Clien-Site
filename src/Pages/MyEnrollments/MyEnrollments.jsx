...
const MyEnrollments = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      setLoading(true);
      axios
        .get(`https://assignment-11-server-site-ashen.vercel.app/my-enrollments?email=${user.email}`)
  ...
  const handleRemove = async (id, courseId) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axios.delete(
          `https://assignment-11-server-site-ashen.vercel.app/enrollments/${id}?courseId=${courseId}`
        );
        setEnrollments(
              ...
              {enrollments.map((enrollment, index) => (
                <tr key={enrollment._id} className="hover:bg-blue-50">
                  ...
                  <td>
                    <button
                      onClick={() =>
                        handleRemove(enrollment._id, enrollment.courseId)
                      }
